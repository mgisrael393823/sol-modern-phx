...
export function MobileNavigation() {
  ...
  return (
    <>
      {/* Mobile Header */}
      <div className="h-16 border-b border-[#E8E3D9] flex items-center justify-between px-4 bg-[#FCFAF5] fixed top-0 left-0 w-full z-40">
        <Link href="/" className="flex-1">
          <h2 className={`${headingFont.className} text-base text-[#333333] tracking-widest`}>SOL MODERN</h2>
          <div className="h-0.5 w-12 bg-[#E57161]"></div>
        </Link>
      ...
      <nav 
        className={`w-4/5 max-w-xs bg-[#FCFAF5] border-r border-[#E8E3D9] h-screen fixed top-0 right-0 z-40 transform transition-transform duration-300 ease-in-out ${
          isOpen ? 'translate-x-0' : 'translate-x-full'
        } overflow-y-auto`}
      >
        <div className="h-full flex flex-col">
          <div className='px-6 py-10 border-b border-[#E8E3D9] flex items-center justify-between'>
            <div>
              <p className={`${bodyFont.className} text-xs text-[#777777] tracking-wide`}>BLUEPRINT CONTENTS</p>
            </div>
            <button onClick={closeMenu} className="text-[#777777]">
              <X size={20} />
            </button>
          </div>
